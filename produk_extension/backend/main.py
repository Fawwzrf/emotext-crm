...
# ─── Endpoints ─────────────────────────────────────────────────────────────────
# Knowledge Base (Directory-based for security, as requested)
import shutil
...
async def upload_knowledge_base(
    ...
    if ext not in ["pdf", "txt"]:
        raise HTTPException(status_code=400, detail="Hanya mendukung PDF dan TXT")
    ...
    try:
        ...
        logger.info(f"[RAG] Berhasil upload dokumen lokal: {file.filename}")
        ...
        return {"status": "success", "message": f"Dokumen {file.filename} berhasil disimpan ke lokal."}
    except Exception as e:
        logger.error(f"[RAG] Gagal upload: {e}")
        ...
        for f in os.listdir(DOC_DIR):
            if f.endswith(".txt") or f.endswith(".pdf"):
                docs.append({
                    "id": f,
                    "filename": f,
                    "created_at": os.path.getctime(os.path.join(DOC_DIR, f)),
                    "chunks": 1, # Not chunking in DB anymore
                    "uploaded_at": str(os.path.getctime(os.path.join(DOC_DIR, f)))
                })
...
async def delete_knowledge_base(
    ...
    if os.path.exists(file_path):
        os.remove(file_path)
        logger.info(f"[RAG] Berhasil hapus dokumen: {filename}")
        
        import rag_service
        rag_service._rag_loaded = False
        threading.Thread(target=_load_local_rag_index, daemon=True).start()
        
        return {"status": "success", "deleted_chunks": 1, "filename": filename}
...
async def stream_suggestion(
    ...
    async def stream_and_save():
        ...
        if full_text:
            msg.suggestion = full_text.strip()
            await db.commit()
